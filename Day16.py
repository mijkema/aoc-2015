...
def main(inp, is_real):
    inp = inp.strip()
    props, aunts = inp.split('\n\n')
    props = dict([p.split(': ') for p in props.split('\n')])
    props = {k: int(v) for k, v in props.items()}
    for a in aunts.split('\n'):
        num, p = re.findall(r'Sue ([0-9]*): (.*)', a)[0]
        num = int(num)
        vals = [x.split(': ') for x in p.split(', ')]
        vals = {k: int(v) for k, v in vals}
        is_aunt = True
