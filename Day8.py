...
def main(inp, is_real):
    inp = inp.strip()
    inp = inp.split('\n')
    total_length = sum(map(len, inp))
    data_length = sum(map(lambda x: len(eval(x)), inp))
    print(total_length - data_length)
    new_length = sum(map(lambda x: 2 + len(x) + x.count('\\') + x.count('\"'), inp))
    print(new_length - total_length)
